...
print(ggdot(data.frame(d20,d2), xyz = c('Expected','Observed'), title = 'HLOD null distribution in the presence of recombination events'))
print(ggdot(data.frame(d30,d3), xyz = c('Expected','Observed'), title = 'HLOD null distribution in the presence of LD and missing data'))
dev.off()
""" | R --slave  
